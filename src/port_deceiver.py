        ...
        while True:
            ...
            if PROTOCOL == 6:
                ...
                if flags == 2 and count == 0:
                    print('receive host scanning packet - syn')
                    # reply ack_rst
                    count = 1
                    reply_tcp_header = self.conn.build_tcp_header_from_reply(5, reply_seq, reply_ack_vum, reply_src_port,
                                                                             reply_dest_port, reply_src_IP, reply_dest_IP,
                                                                             port_flag)

                elif flags == 2 and count == 1:
                    print('receive port scanning packet - syn')
                    # reply ack_rst
                    count = 0
                    reply_tcp_header = self.conn.build_tcp_header_from_reply(5, reply_seq, reply_ack_vum, reply_src_port,
                                                                             reply_dest_port, reply_src_IP, reply_dest_IP,
